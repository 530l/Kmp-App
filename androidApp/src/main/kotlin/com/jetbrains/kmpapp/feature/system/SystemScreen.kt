...
fun SystemScreen(
    ...
    val leaves: List<Chapter> = remember(state.tree) {
        state.tree.flatMap { it.children }
    }

    val pagerState = rememberPagerState(initialPage = state.selectedIndex, pageCount = { leaves.size })
    ...
    // Bug 1 修复：Pager 切页时，Tab 条自动滚动到选中项
    LaunchedEffect(pagerState.currentPage, leaves.size) {
    ...
    Column(Modifier.fillMaxSize()) {
        ...
        if (leaves.isEmpty()) {
            ...
            HorizontalPager(
                ...
                val cid = leaves[page].id
                val chapter = state.articlesMap[cid] ?: ChapterArticles()
                ChapterContent(
                    chapter = chapter,
    ...
    PullToRefreshBox(
        ...
        when {
            ...
            chapter.error != null && chapter.articles.isEmpty() -> {
                ...
                modifier = Modifier.fillMaxSize(),
                state = listState,
                // Bug 3 修复：底部多留空间，防止最后一个 item 被底部导航栏遮挡
...
private fun ChapterTabRow(
    ...
    LazyRow(
        ...
        items(leaves, key = { it.id }) { chapter ->
            val index = leaves.indexOf(chapter)
